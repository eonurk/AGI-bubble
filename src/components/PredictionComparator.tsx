...
export function PredictionComparator({ years }: PredictionComparatorProps) {
    ...
    const [result, setResult] = useState<{ percentile: number; isOptimist: boolean; message: string } | null>(null);
    ...
    const calculatePosition = () => {
        ...
        if (year < 2027) message = "You're an Accelerationist! Very few expect it this soon.";
        else if (year < 2032) message = "You're with the majority. This is the consensus decade.";
        else if (year < 2040) message = "You're a Skeptic. You think the current hype will hit a wall.";
        else message = "You're a Long-termist. You believe we have a long road ahead.";
    ...
    const shareResult = async () => {
        if (!result) return;
        const text = `I predict AGI in ${userYear}. That makes me an ${result.isOptimist ? 'Accelerationist' : 'Skeptic'} (faster than ${result.percentile}% of the community). Where do you stand? #AGIBubble ${window.location.href}`;
        try {
            ...
            <CardContent>
                <div className="flex gap-4 items-end">
                    <div className="flex-1 space-y-2">
                        <label className="text-sm font-medium text-muted-foreground">Your Prediction (Year)</label>
                ...
                {result && (
                    ...
                    >
                        <div className="flex items-start gap-4">
                            <div className={`p-2 rounded-full ${result.isOptimist ? "bg-green-500/10 text-green-500" : "bg-orange-500/10 text-orange-500"}`}>
                                {result.isOptimist ? <Trophy className="w-5 h-5" /> : <Clock className="w-5 h-5" />}
                            </div>
                        ...
                        <div className="flex justify-between items-center pt-2">
                            ...
                            <Button variant="ghost" size="sm" className="h-8 text-xs gap-2 hover:text-primary hover:bg-primary/10" onClick={shareResult}>
